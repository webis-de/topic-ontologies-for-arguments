...
import csv
path_wikipedia= get_path_source('ontology-wikipedia')
...
def save_topics():
    ...
    for index,row in df_wikipedia.iterrows():
        ...
        columns['url'].append("")
        columns['id'].append("3.%d"%topic_index)
        topic_index = topic_index+ 1
    columns['ontology']= ["wikipedia" for topic in columns['name']]
    topics_df  = pd.DataFrame(columns)
    topics_df.set_index('id')
    path_topics= get_path_topics('ontology-wikipedia')
